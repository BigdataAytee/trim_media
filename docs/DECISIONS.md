...
- D0.5 — The Hub's headline number is rounded according to the estimate's confidence: one decimal place only when the estimate is `PROBED`, whole units at `PREDICTED` and `SEED` (`screens.md` §1). Before any probe the band is ±18%, so a rendered "12.4 GB" is a fiction the word "about" is not strong enough to carry. This is the first use anywhere in the UI of `EstimateRange.confidence`, which the model has carried since M1 and nothing has read. The headline also names the base the saving comes from ("from 23 videos that use 36.2 GB now") rather than a bare percentage, taken as the stronger reading of §7.3's "savings always shown as both size and percent".
